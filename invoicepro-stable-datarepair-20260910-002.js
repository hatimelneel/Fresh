...
function normalizeDB(){
  ...
  const objectsOnly=(arr)=>arr.filter(x=>x && typeof x==="object" && !Array.isArray(x));
  db.customers=objectsOnly(db.customers);
  db.invoices=objectsOnly(db.invoices);
  db.quotes=objectsOnly(db.quotes);
  db.products=objectsOnly(db.products);
  db.expenses=objectsOnly(db.expenses);
  db.customerPayments=objectsOnly(db.customerPayments);

  db.customers.forEach(c=>{
    if(!c.id) c.id=makeCustomerId();
    c.name=text(c.name);
    c.phone=text(c.phone);
    c.address=text(c.address);
  });

  db.invoices.forEach((inv,i)=>{
    inv.number=text(inv.number)||("INV-"+String(i+1).padStart(5,"0"));
    inv.customer=text(inv.customer);
    inv.phone=text(inv.phone);
    inv.address=text(inv.address);
    inv.customerId=text(inv.customerId);
    inv.status=text(inv.status)||"draft";
    inv.sourceQuoteNumber=text(inv.sourceQuoteNumber);
    inv.notes=text(inv.notes);
    inv.terms=text(inv.terms);
    inv.date=text(inv.date)||new Date().toISOString();

    if(!Array.isArray(inv.items)) inv.items=[];
    inv.items=objectsOnly(inv.items).map(item=>({
      ...item,
      name:text(item.name),
      qty:+item.qty||0,
      price:+item.price||0,
    ...
    inv.paid=+inv.paid||0;
    inv.balance=inv.balance===undefined || inv.balance===null
      ? Math.max(inv.total-inv.paid,0)
      : (+inv.balance||0);

    if(!inv.customerId){
      const invName=inv.customer.trim().toLowerCase();
      const invPhone=inv.phone.trim();
      const match=db.customers.find(c=>
        (invName && c.name.trim().toLowerCase()===invName) ||
        (invPhone && c.phone.trim()===invPhone)
      );
      if(match) inv.customerId=match.id;
    }
  });

  db.quotes.forEach((q,i)=>{
    q.number=text(q.number)||("QT-"+String(i+1).padStart(5,"0"));
    q.customer=text(q.customer);
    q.phone=text(q.phone);
    q.address=text(q.address);
    q.customerId=text(q.customerId);
    q.status=text(q.status)||"draft";
    q.convertedInvoiceNumber=text(q.convertedInvoiceNumber);
    q.convertedInvoiceDate=text(q.convertedInvoiceDate);
    q.notes=text(q.notes);
    q.terms=text(q.terms);
    q.date=text(q.date)||new Date().toISOString();
    q.validity=+q.validity||30;

    if(!Array.isArray(q.items)) q.items=[];
    q.items=objectsOnly(q.items).map(item=>({
      ...item,
      name:text(item.name),
      qty:+item.qty||0,
      price:+item.price||0,
    ...
    if(!q.customerId){
      ...
      const qPhone=q.phone.trim();
      const match=db.customers.find(c=>
        (qName && c.name.trim().toLowerCase()===qName) ||
        (qPhone && c.phone.trim()===qPhone)
      );
  ...
  db.expenses.forEach(e=>{
    ...
    e.date=text(e.date)||new Date().toISOString();
    e.documentType=text(e.documentType);
    e.documentNumber=text(e.documentNumber);
    e.amount=+e.amount||0;
  });
  ...
  db.customerPayments
    ...
    .forEach(p=>{
      p.id=text(p.id)||("PAY-"+Date.now()+"-"+Math.random().toString(36).slice(2,6));
      p.customerId=text(p.customerId);
      p.customerName=text(p.customerName);
      p.customerPhone=text(p.customerPhone);
      p.date=text(p.date)||new Date().toISOString().slice(0,10);
      p.method=text(p.method)||"cash";
      p.reference=text(p.reference);
      p.note=text(p.note);
      p.amount=+p.amount||0;

      if(!Array.isArray(p.allocations)) p.allocations=[];
      p.allocations=objectsOnly(p.allocations).map(a=>({
        invoiceNumber:text(a.invoiceNumber),
        amount:+a.amount||0
      }));
      ...
      p.receiptNo=text(p.receiptNo);
      if(!p.receiptNo){
        while(db.customerPayments.some(x=>x!==p && text(x.receiptNo)==="RCPT-"+String(receiptCounter).padStart(5,"0"))){
          receiptCounter++;
        }
  ...
  db.settings.company=text(db.settings.company);
  db.settings.phone=text(db.settings.phone);
  db.settings.email=text(db.settings.email);
  db.settings.address=text(db.settings.address);
  db.settings.vat=text(db.settings.vat);
  db.settings.currency=text(db.settings.currency)||"OMR";

  // Save the repaired form immediately so the next opening uses clean data.
  try{
    localStorage.setItem(KEY,JSON.stringify(db));
  }catch(e){
    console.error("Could not persist repaired data:",e);
  }
}
normalizeDB();
...
const DEFAULT_LOGO="smart-gate-logo-stable-20260910.jpeg";
